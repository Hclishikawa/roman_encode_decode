...
# Keep the above syntax directive! It's used to enable Docker BuildKit

#checkov:skip=CKV_DOCKER_2:Healthcheck is not required for batch images.
...
####################################################################
# BUILDER-BASE: Used to build deps + create our virtual environment
####################################################################
...
COPY tests/ tests/
COPY roman_encode_decode/ roman_encode_decode/
...
RUN --mount=type=cache,target=/root/.cache poetry install --no-root --with test
...
###########################################
# PRODUCTION: Final image used for runtime
###########################################
...
EXPOSE 8080
USER appuser
CMD ["python", "roman_encode_decode/main.py"]